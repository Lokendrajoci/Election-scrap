...
from scraper import fetch_raw_records, fetch_total_votes

app = Flask(__name__)

CACHE_TTL_SECONDS = 600
_cache_df = None
_cache_total_votes = None
...
def get_votes_df(force_refresh: bool = False):
    ...
    global _cache_df, _cache_total_votes, _cache_expires_at
    ...
    if force_refresh or not cache_valid:
        ...
        _cache_total_votes = fetch_total_votes(records)
        _cache_df = process_party_votes(records, _cache_total_votes)
        _cache_expires_at = now + timedelta(seconds=CACHE_TTL_SECONDS)

    return _cache_df, _cache_total_votes


@app.route("/")
def home():
    df, total_votes = get_votes_df()
    top10 = df.head(10)
...
def api_votes_refresh():
    df, _ = get_votes_df(force_refresh=True)
    return jsonify(df.to_dict(orient="records"))
